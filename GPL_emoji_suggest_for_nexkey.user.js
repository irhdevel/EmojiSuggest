// ==UserScript==
// @name         EmojiSuggest - Nexkey
// @namespace    irihi_design
// @version      0.2.9
// @description  ※割とやばい処理の書き方です※ - Nexkeyで自分の居るインスタンスにない絵文字をクリックした際、検索欄に絵文字名を入力し類似の絵文字を検索します。ご自分のインスタンスをmatchに設定するなどしてからご利用ください。
// @author       Irihi_design
// @match        https://social.sda1.net/*
// @grant        none
// @run-at       document-idle
// @updateURL    https://github.com/irhdevel/EmojiSuggest/raw/main/GPL_emoji_suggest_for_nexkey.user.js
// @downloadURL  https://github.com/irhdevel/EmojiSuggest/raw/main/GPL_emoji_suggest_for_nexkey.user.js
...
(function() {
    ...
    function inputEmojiId(name){
        let searchInput = document.querySelectorAll(".popup.qzhlnise > .content > .omfetrab > input.search[data-prevent-emoji-insert][type=\"search\"],  .drawer.qzhlnise > .content.fixed > .omfetrab > input.search[data-prevent-emoji-insert][type=\"search\"]")
    ...
    function errorDialog(msg){
        alert(msg + "バグもしくはNexkeyのアップデートによりUserScriptが壊れています。\n\n==複数回試してもこのエラーが連発する場合==\n修正のアップデートを確認し、なければ今すぐこのUserScriptを無効化してください。")
    }
        ...
        MutationList.forEach(
            (m)=>{
                m.addedNodes.forEach((e)=>{
                    if(e.classList && e.classList.contains("note")){
                        // ノートを詳細表示時のDOM変更
                        // V13で消えたみたい
                        setOnclickAttr()
                    } else if(e.classList && e.classList.contains("tkcbzcuz") && e.classList.contains("qtqtichx")) {
                        // ノートが流れてくる時のDOM変更
                        setOnclickAttr()
                    } else if(e.classList && e.classList.contains("hkzvhatu") && e.classList.contains("_button")) {
                        // ノートが流れてくる前に絵文字が追加されたとき用の、絵文字追加時のDOM変更
                        setOnclickAttr()
                    } else if(e.classList && e.classList.contains("tl") && e.querySelector(".giivymft > .notes") != null) {
                        // 初回リスト読み込み時
                        setOnclickAttr()
                    }
                }
